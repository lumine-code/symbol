...
module.exports = class Registry {
  ...
  invalidateProvider(provider, editor = null) {
    ...
    for (let someEditor of editors) {
      ...
      if (provider.isExclusive) {
        ...
        // contender changes, the whole winner selection must run again. Keep
        // the last complete bundle only as a display fallback; runFileFetch
        // deliberately does not seed the replacement run with it.
        let providers = this.invalidatedProviders.get(someEditor);
        if (!providers) {
          providers = new Set();
          this.invalidatedProviders.set(someEditor, providers);
        }
  ...
  async runFileFetch(editor, signal, { listController = null } = {}) {
    ...
    const reselectingExclusive = [...(onlyProviders ?? [])].some(
      (provider) => provider.isExclusive,
    );
    const selectingOnlyInvalidated =
      onlyProviders?.size && [...onlyProviders].every((provider) => !provider.isExclusive);
    let existing = reselectingExclusive ? null : (existingBundle?.flat ?? null);
    let providers;
    if (selectingOnlyInvalidated) {
    ...
    if (selectingOnlyInvalidated && providers.length > 0) {
      providers = providers.filter((p) => onlyProviders.has(p));
    }

    if (providers.length === 0) {
      this.invalidatedProviders.delete(editor);
      if (existingBundle && !reselectingExclusive) return existingBundle;
      let unavailable = { flat: [], tree: null, unavailable: true };
      this.cache.set(editor, unavailable);
      return unavailable;
    }

    let gathered = await this.gatherSymbols(providers, signal, meta, {
    ...
    if (
      !gathered.complete &&
      [...gathered.failedProviders].some((provider) => provider.isExclusive)
    ) {
      // A throw, malformed return or timeout is not the same answer as an
      // empty symbol list. Keep the last complete bundle visible when there is
      // one, and leave this provider stale so a later request can recover.
      return existingBundle;
    }

    let symbols = gathered.symbols;
    symbols.sort((a, b) => a.position.compare(b.position));
    let bundle = this.createFileBundle(symbols);
    // A complete empty answer is still an answer. Cache it so consumers do not
    // ask again; an incomplete supplemental run remains marked stale below.
    this.cache.set(editor, bundle);
    if (gathered.complete) {
      this.invalidatedProviders.delete(editor);
    } else {
      this.invalidatedProviders.set(editor, gathered.failedProviders);
    }
    return bundle;
  }
   ...
   * @returns {Promise<Array|Object>} The gathered symbols, or their completion
   *   status as well when `withStatus` is requested.
   */
  async gatherSymbols(
    providers,
    signal,
    ...
    if (tasks.length > 0) {
      ...
      if (!settled) {
        complete = false;
        for (const provider of pendingProviders) failedProviders.add(provider);
      }
    }
    closed = true;

    return withStatus ? { symbols, complete, failedProviders } : symbols;
  }
};

...
module.exports = class Registry {
  constructor() {
    ...
    // so the picker, outline, and breadcrumbs never issue parallel fetches.
    this.cache = new Map();
  ...
  removeProviderFromCache(editor, provider) {
    ...
    let results = bundle.flat.filter((sym) => sym.providerId !== provider.packageName);
  ...
  getFileSymbols(editor, options = {}) {
    return this.getFileBundle(editor, options).then((bundle) => bundle?.flat ?? null);
  }

  getFileSymbolTree(editor, options = {}) {
    return this.getFileBundle(editor, options).then((bundle) => bundle?.tree ?? null);
  }
  ...
  createFileBundle(symbols) {
    return { flat: symbols, tree: this.buildFileSymbolTree(symbols) };
  }
  ...
  buildFileSymbolTree(symbols) {
    ...
    for (let index = 0; index < entries.length; index++) {
      ...
      // Prefer structural containment. When several earlier ranges contain
      // this one, keep the smallest (the nearest lexical parent).

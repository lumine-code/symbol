...
class InvalidProviderError extends Error {
  constructor(faults, provider) {
    let packageName = provider.packageName
      ...
      : "a symbol provider";
    let message = `symbol failed to consume ${packageName} because certain properties are invalid: ${faults.join(", ")}. Please fix these faults or contact the package author.`;
    super(message);
...
module.exports = class ProviderBroker {
   ...
   *   of symbol providers.
   */
  async select(meta, { enforceExclusivity = true, candidates = null } = {}) {
    if (this.destroyed) return [];
    let shouldLog = Config.get("enableDebugLogging");
    ...
    // Provider registration may change while asynchronous answers settle.
    // Select against one snapshot so every outcome keeps the provider index
    // it was created for.
    const providers = candidates
      ? candidates.filter((provider) => this.providers.includes(provider))
      : [...this.providers];
    if (providers.length === 0) return [];
    let deadlineTimer;
    const deadline = new Promise((resolve) => {
      deadlineTimer = setTimeout(resolve, CAN_PROVIDE_SYMBOLS_TIMEOUT, TIMED_OUT);
    });
    let answers = providers.map((provider) => {
      if (shouldLog) console.debug(`Asking provider:`, provider.name, provider);
      return Promise.race([
        Promise.resolve().then(() => provider.canProvideSymbols(meta)),
        deadline,
      ]);
    });
    ...
    for (let [index, provider] of providers.entries()) {
      ...
      // The contract clamps scores above 1: there is no winning a number war,
      // and the user breaks ties in the settings.
      if (score > 1) score = 1;

...
function buildService(registry) {
  return Object.freeze({
    ...
    getFileSymbolTree: (editor, options) => registry.getFileSymbolTree(editor, options),
...
module.exports = {
  activate() {
    ...
    this.workspaceSubscription = lumine.commands.add("lumine-workspace", {
      "symbol:toggle-project-symbols": {
        description: "Search the symbols of every file in the project.",
        didDispatch: (event) => {
    ...
    this.editorSubscription = lumine.commands.add("lumine-text-editor:not([mini])", {
      ...
      "symbol:go-to-declaration": {
        description: "Jump to where the symbol under the cursor is declared.",
        didDispatch: () => {
      ...
      "symbol:return-from-declaration": {
        description: "Go back to where the last declaration jump started.",
        didDispatch: () => {
          if (!this.ensureProvidersExist()) return;
  ...
  getSelectedTextIfEnabled(event) {
    const editor = lumine.textEditors.getTextEditorForElement(event?.target, {
      includeMini: false,
    });
    if (!editor) return "";

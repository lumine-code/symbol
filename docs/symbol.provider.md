...
| `onShouldClearCache(callback)` | Invoke the callback when your symbols go stale for a reason `symbol` cannot see. It already clears on config change, provider activation, grammar change, save, and buffer change.    |
